...
interface Message {
  role: 'user' | 'assistant';
  content: string;
  apiContent?: string; // Content including hidden constraints for the API
...
export default function Proto2Page() {
  ...
  // Handle click outside to collapse in chat mode
  useEffect(() => {
  ...
  const generateFacets = useCallback(
    async function gf(promptText: string, attempts = 0) {
      ...
      try {
        const res = await fetch('/api/proto2/generate-facets', {
          method: 'POST',
  ...
  useEffect(() => {
    if (isChatting || hasStartedChat) return;
  ...
  const handleRetry = async () => {
    ...
    // Keep messages only up to the last user message
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-manrope relative overflow-hidden">
      <div
        ...
        style={{
          background: 'linear-gradient(to bottom, #FFF5C2 0%, #FFFFFF 30%)',
        }}
      />
