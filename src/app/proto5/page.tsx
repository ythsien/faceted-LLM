...
interface Message {
  ...
  generatedFacets?: Record<string, string[]>;
  isGeneratingFacets?: boolean;
}

export default function Proto5Page() {
  ...
  const latestGeneratedFacetsRef = useRef<Record<string, string[]> | null>(
    null
  );
  ...
  const generateFacets = async (
    ...
    setMessages((prev) => {
      const updated = [...prev];
      updated[msgIndex] = { ...updated[msgIndex], isGeneratingFacets: true };
      return updated;
    });

    try {
      // Send history up to the user message before this assistant message
      const historyForFacets = history.slice(0, msgIndex);

      const res = await fetch('/api/proto5/generate-facets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: historyForFacets.map((m) => ({
            role: m.role,
      ...
      if (!res.ok) {
        if (res.status === 503 && attempts < 2) {
          ...
          return generateFacets(msgIndex, history, attempts + 1);
        }
        setMessages((prev) => {
          const updated = [...prev];
          updated[msgIndex] = {
            ...updated[msgIndex],
            isGeneratingFacets: false,
          };
          return updated;
        });
        return;
      ...
      if (data.facets) {
        latestGeneratedFacetsRef.current = data.facets;
        setMessages((prev) => {
          const updated = [...prev];
          updated[msgIndex] = {
            ...updated[msgIndex],
            generatedFacets: data.facets,
            isGeneratingFacets: false,
          };
          return updated;
        });
      ...
      setMessages((prev) => {
        const updated = [...prev];
        updated[msgIndex] = { ...updated[msgIndex], isGeneratingFacets: false };
        return updated;
      });
    }
  };
  ...
  const performChat = async (
    ...
    try {
      ...
      onRenderComplete(
        lastUserPrompt,
        latestGeneratedFacetsRef.current,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    onSubmitStart();
    latestGeneratedFacetsRef.current = null;
  ...
  const handleUpdateResponse = async (index: number) => {
    ...
    // Retrieve the existing generated facets from the message to preserve them
    const existingMsg = messages[index];
    latestGeneratedFacetsRef.current = existingMsg?.generatedFacets || null;
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-manrope relative overflow-hidden text-black">
      <div
        ...
        style={{
          background: 'linear-gradient(to bottom, #F3E5F5 0%, #FFFFFF 40%)',
        }}
      />
